...
export const sharedRackLedgerScorecardMarkup = `
  <main class="scorecard-app" data-shared-rack-ledger-scorecard data-primary-scoring>
    ...
    <nav class="score-hub-links" aria-label="League night" style="display:flex;flex-wrap:wrap;gap:8px;padding:0 0 8px">
      ...
      <a href="/messages" style="min-height:44px;display:inline-flex;align-items:center;padding:0 12px;border:1px solid currentColor;border-radius:10px;opacity:.85;text-decoration:none;color:inherit">Messages</a>
    </nav>
...
export const sharedRackLedgerScorecardControllerSource = String.raw`
  (function(){
    ...
    function setStatus(message,tone,opts){if(window.fdSetStatus){window.fdSetStatus(statusEl,message,tone||'',opts||{});return}statusEl.textContent=message;statusEl.dataset.tone=tone||''}
