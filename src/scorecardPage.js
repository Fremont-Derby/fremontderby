...
<head>
  ...
  <style>
    ...
    * { box-sizing:border-box; }
    ...
    button:focus-visible,a:focus-visible { outline:3px solid #bcebd0; outline-offset:2px; }
    ...
    .opening-setup { margin-bottom:9px; padding:10px; border:1px solid var(--line); border-radius:14px; background:#151818; display:grid; gap:8px; }
    .opening-copy { display:flex; justify-content:space-between; align-items:baseline; gap:10px; }
    .opening-copy strong { font-size:.88rem; }
    .opening-copy span { color:var(--muted); font-size:.7rem; text-align:right; }
    .opening-options { display:grid; grid-template-columns:1fr 1fr; gap:8px; }
    .opening-option { min-height:48px; background:#222826; color:#f6f1e7; border-color:var(--line); }
    .opening-option[aria-pressed="true"] { background:var(--gold); color:#191305; border-color:var(--gold); }
    .opening-setup[data-locked="true"] .opening-copy span { color:#9ee5bd; }
    ...
    @media (max-width:520px) {
      ...
      .opening-copy { align-items:flex-start; }
      .opening-copy span { max-width:45%; }
...
<body>
  <main class="app">
    ...
    <section class="opening-setup" data-opening-setup aria-label="Opening game order"><div class="opening-copy"><strong>Which game first?</strong><span data-opening-note>Choose before rack 1. Racks 1–3 use this game.</span></div><div class="opening-options"><button class="opening-option" data-opening="8-ball" type="button" aria-pressed="false">8 first</button><button class="opening-option" data-opening="9-ball" type="button" aria-pressed="false">9 first</button></div></section>
  ...
  <script>
    ...
    function renderOpeningChoice(selected,locked){openingSetup.dataset.locked=String(locked); for(const button of openingButtons){button.setAttribute('aria-pressed',String(button.dataset.opening===selected)); button.disabled=locked;} openingNote.textContent=locked?'Order locked after rack 1 · '+(selected==='9-ball'?'9-ball first':'8-ball first'):'Choose before rack 1. Racks 1–3 use this game.';}
    ...
    async function chooseOpeningDiscipline(openingDiscipline){setStatus('Saving game order…'); await api('/api/player-matches/:id/score-racks',{method:'POST',body:JSON.stringify({openingDiscipline,scoringTeamId})}); await loadAll(); setStatus((openingDiscipline==='9-ball'?'9-ball':'8-ball')+' will be played first.','ok');}
    ...
    for(const button of openingButtons) button.addEventListener('click',()=>run(()=>chooseOpeningDiscipline(button.dataset.opening))); rackAButton.addEventListener('click',()=>run(()=>recordRack('A'))); rackBButton.addEventListener('click',()=>run(()=>recordRack('B'))); undoButton.addEventListener('click',()=>run(undoRack)); confirmButton.addEventListener('click',()=>run(confirmScore)); finalizeButton.addEventListener('click',()=>run(finalizeMatch));
    if(!matchId||!scoringTeamId){setStatus('Choose a match and scoring team from the scorecard list.','error'); for(const button of openingButtons)button.disabled=true; rackAButton.disabled=true; rackBButton.disabled=true; undoButton.disabled=true; confirmButton.disabled=true; finalizeButton.disabled=true;} else if(!accessToken()){setStatus('Sign in with Google to score this match.','error'); for(const button of openingButtons)button.disabled=true; const context=document.querySelector('[data-context]'); context.innerHTML='<a href="/profile">Open Profile to sign in</a>';} else {run(loadAll); startRefresh();}
  </script>
</body>
</html>`;
}
