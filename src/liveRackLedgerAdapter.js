  ...
  (function(){
    ...
    window.fdRackLedgerAdapter={
      ...
      async load(){
        ...
        lastOwnRacks=Array.isArray(comparisonBody.comparison?.own_racks)?comparisonBody.comparison.own_racks:[];
      ...
      async saveRack(input){
        const body={scoringTeamId,winnerSide:input.winnerSide,expectedRacks:lastOwnRacks};
      ...
      async undo(){return api('/api/player-matches/:id/score-racks/undo',{method:'POST',body:JSON.stringify({scoringTeamId,expectedRacks:lastOwnRacks})})},
      async confirm(){return api('/api/player-matches/:id/score-confirm',{method:'POST',body:JSON.stringify({scoringTeamId,expectedRacks:lastOwnRacks})})},
