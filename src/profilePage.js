...
<body>
    ...
    <section class="grid">
      <article class="panel">
        ...
        <form class="stack" data-auth-form>
          <label>Email
            <input name="email" data-email type="email" autocomplete="email" />
          </label>
          <label>Password
            <input name="password" data-password type="password" autocomplete="current-password" />
          </label>
          <div class="actions">
            <button class="primary" data-sign-in type="submit">Sign in</button>
            <button class="secondary" data-sign-up type="button">Create account</button>
          </div>
  ...
  <script>
    ...
    function setToken(value) {
      ...
      sessionState.textContent = value ? 'Signed in' : 'Signed out';
    }
    ...
    function renderProfile(profile) {
      ...
      document.querySelector('[data-rating]').textContent = profile && profile.fargo_rating != null
        ? String(profile.fargo_rating)
        : '-';
      document.querySelector('[data-rating-status]').textContent = profile && profile.rating_status
        ? profile.rating_status
        : 'unverified';
      ...
      renderRows(
        ...
        teamEmpty,
        teamCount,
        profile && Array.isArray(profile.teams) ? profile.teams : [],
        (row) => [row.seasonName, row.teamName, row.role],
      );
      renderRows(
        seasonBody,
        seasonEmpty,
        seasonCount,
        profile && Array.isArray(profile.seasons) ? profile.seasons : [],
        (row) => [row.seasonName, row.participationType, row.status],
      );
    }
    ...
    async function signIn() {
      ...
      const body = await authRequest('/auth/v1/token?grant_type=password', requireCredentials());
    ...
    async function signUp() {
      ...
      const body = await authRequest('/auth/v1/signup', requireCredentials());
      setToken(body.access_token);
      if (body.access_token) {
        await loadProfile();
      } else {
        setStatus('Check email before signing in', 'ok');
      }
    }
    ...
    signUpButton.addEventListener('click', () => run(signUp));
