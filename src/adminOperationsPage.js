...
<head>
  ...
  <style>
    :root{color-scheme:dark;font-family:Inter,ui-sans-serif,system-ui,sans-serif;background:#07150f;color:#f4f7f5;--panel:#0b2418;--line:#315d45;--muted:#afc1b6;--green:#39b979;--gold:#e9bd45;--danger:#d95d53}*{box-sizing:border-box}body{margin:0;min-height:100vh;background:radial-gradient(circle at 50% 0,#123b28,#07150f 34rem)}button{font:inherit}.app{width:min(1120px,100%);margin:auto;padding:18px 16px 36px}.head{display:flex;justify-content:space-between;align-items:end;gap:14px;margin-bottom:16px}.head h1{margin:0;font-size:clamp(1.8rem,7vw,3rem)}.sub{color:var(--muted);line-height:1.5}.head-actions{display:grid;gap:8px;justify-items:end}.admin-links{display:flex;gap:10px;flex-wrap:wrap}.admin-links a{color:#bcebd0;font-weight:850}.status{padding:10px 12px;border:1px solid var(--line);border-radius:10px;background:#0a1c13;color:var(--muted)}.status[data-tone="critical"]{border-color:#98483f;color:#ffb3ac}.status[data-tone="warning"]{border-color:#8f722a;color:#f4dc97}.status[data-tone="healthy"]{border-color:#357a54;color:#a9e6c2}.metrics{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:10px;margin:14px 0}.metric,.panel{border:1px solid var(--line);border-radius:13px;background:var(--panel)}.metric{padding:13px;display:grid;gap:6px}.metric span{color:var(--muted);font-size:.72rem;font-weight:850;text-transform:uppercase}.metric strong{font-size:1.55rem}.layout{display:grid;grid-template-columns:minmax(0,1.2fr) minmax(0,.8fr);gap:12px}.panel{padding:14px}.panel h2{margin:0 0 12px;font-size:1.05rem}.actions,.detail-grid{display:grid;gap:9px}.action{padding:12px;border:1px solid var(--line);border-left-width:5px;border-radius:10px;background:#081a12}.action[data-severity="critical"]{border-left-color:var(--danger)}.action[data-severity="warning"]{border-left-color:var(--gold)}.action[data-severity="healthy"]{border-left-color:var(--green)}.action strong{display:block}.action p{margin:4px 0;color:var(--muted);line-height:1.4}.action a{color:#bcebd0;font-weight:850}.row{display:flex;justify-content:space-between;gap:16px;padding:9px 0;border-bottom:1px solid var(--line)}.row:last-child{border-bottom:0}.row span{color:var(--muted)}.empty{padding:18px;border:1px dashed var(--line);border-radius:10px;color:var(--muted)}button{min-height:42px;padding:0 13px;border:1px solid var(--line);border-radius:10px;background:#173f2a;color:#f4f7f5;font-weight:850;cursor:pointer}@media(max-width:760px){.app{padding:12px 10px 28px}.head{display:grid}.head-actions{justify-items:start}.metrics{grid-template-columns:1fr 1fr}.layout{grid-template-columns:1fr}.status{text-align:left}}@media(max-width:420px){.metrics{grid-template-columns:1fr}}
  </style>
</head>
<body>
  <main class="app">
    <header class="head"><div><h1>League operations</h1><div class="sub" data-season>Is Fremont Derby running smoothly?</div></div><div class="head-actions"><nav class="admin-links" aria-label="Admin tools"><a href="/season-setup">Season setup</a><a href="/messages/moderation">Moderation</a></nav><div class="status" data-status>Loading…</div><button data-refresh type="button">Refresh</button></div></header>
  ...
  <script>
    ...
    async function api(retry=true){if(!token())throw new Error('Sign in with a league admin account.');const response=await fetch('/api/admin/operations',{headers:{authorization:'Bearer '+token()}});if(response.status===401&&retry&&await refreshSession())return api(false);const body=await parseJson(response);if(!response.ok)throw new Error(body.error||'Request failed');return body}
    ...
    function render(overview){statusEl.textContent=overview.overall==='healthy'?'All monitored checks are healthy':overview.overall==='critical'?'Critical attention required':'Review recommended';statusEl.dataset.tone=overview.overall;seasonEl.textContent=overview.season?overview.season.name+' · '+overview.season.status:'No current season';for(const element of document.querySelectorAll('[data-count]'))element.textContent=value(overview.counts[element.dataset.count]);actionsEl.replaceChildren();if(!overview.actions.length){const card=document.createElement('div');card.className='action';card.dataset.severity='healthy';const title=document.createElement('strong');title.textContent='No action needed';const detail=document.createElement('p');detail.textContent='All currently monitored checks are healthy.';card.append(title,detail);actionsEl.append(card)}for(const item of overview.actions){const card=document.createElement('article');card.className='action';card.dataset.severity=item.severity;const title=document.createElement('strong');title.textContent=item.title;const detail=document.createElement('p');detail.textContent=item.detail;card.append(title,detail);if(item.href){const link=document.createElement('a');link.href=item.href;link.textContent='Open';card.append(link)}actionsEl.append(card)}renderRows(document.querySelector('[data-league]'),[['Rounds',overview.counts.rounds],['Team matchups',overview.counts.teamMatches],['Lineups',overview.counts.lineups],['Player matches',overview.counts.playerMatches],['Live',overview.counts.liveMatches],['Finalized',overview.counts.finalizedMatches],['Forfeits',overview.counts.forfeits]]);renderRows(document.querySelector('[data-communication]'),[['Team',overview.counts.teamMessages],['Direct',overview.counts.directMessages],['League',overview.counts.leagueMessages],['Matchup',overview.counts.matchupMessages],['Open reports',overview.counts.openReports]]);renderRows(document.querySelector('[data-system]'),[['Rating records',overview.counts.ratings],['Latest rating update',formatTime(overview.rating.latestUpdatedAt)],['Environment',overview.environment.environment],['Supabase project',overview.environment.supabase.projectRef||'Unknown'],['Binding health',overview.environment.ok?'Healthy':'Needs attention']])}
    async function load(){statusEl.textContent='Loading…';statusEl.dataset.tone='';try{const body=await api();render(body.overview)}catch(error){statusEl.textContent=error.message;statusEl.dataset.tone='critical';actionsEl.innerHTML='<div class="empty">This page is available to league admins only. Sign in through Profile and try again.</div>'}}
