...
<body>
  ...
  <script>
    ...
    function renderFailure(error){const msg=error&&error.status===403?'League admin access required':(window.fdFriendlyError?window.fdFriendlyError(error):(error&&error.message)||'Could not load operations');statusEl.textContent=msg;statusEl.dataset.tone='error';actionsEl.replaceChildren();const card=document.createElement('div');card.className='empty';card.textContent=error.status===403?'This page is available only to league admins.':'Operations could not load. No league data was changed.';const recovery=document.createElement('div');recovery.className='recovery';const profile=document.createElement('a');profile.href='/profile';profile.textContent='Open Profile';recovery.append(profile);const score=document.createElement('a');score.href='/scorecard';score.textContent='Score hub';recovery.append(score);if(error.status!==403){const retry=document.createElement('a');retry.href='/admin/operations';retry.textContent='Try again';recovery.append(retry)}actionsEl.append(card,recovery)}
