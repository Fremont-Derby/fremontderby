...
let bestLive=null,bestLiveScore=-Infinity,bestUpcoming=null,bestUpcomingDistance=Infinity,bestFinal=null,bestFinalStage=-1;for(const round of rounds){const matches=round.matches||[];let hasLive=false,hasFinal=false;for(const match of matches){if(match.status==='in_progress')hasLive=true;if(match.status==='finalized'||match.status==='corrected')hasFinal=true}if(round.status==='finalized')hasFinal=true;const stage=round.stage==='championship'?4:round.stage==='semifinal'?3:round.stage==='tiebreaker'?2:1;const parsed=Date.parse(round.scheduledOn||'');const distance=Number.isFinite(parsed)?Math.abs(parsed-now):Number.MAX_SAFE_INTEGER;if(hasLive&&!seasonComplete){const score=stage*1e15-distance;if(score>bestLiveScore){bestLiveScore=score;bestLive=round.roundId}}if(seasonComplete&&hasFinal&&stage>=bestFinalStage){bestFinalStage=stage;bestFinal=round.roundId}if(!round.scheduledOn||round.scheduledOn>=today){if(distance<bestUpcomingDistance){bestUpcomingDistance=distance;bestUpcoming=round.roundId}}}if(bestLive)return bestLive;if(seasonComplete&&bestFinal)return bestFinal;return bestUpcoming||(rounds[rounds.length-1]||{}).roundId||''}
    ...
    function renderRound(){const round=rounds.find((item)=>item.roundId===roundSelect.value);if(!round){panel.hidden=true;return}const today=new Date().toISOString().slice(0,10);roundKicker.textContent=round.scheduledOn===today?'Tonight':(round.scheduledOn&&round.scheduledOn>today?'Next league night':'League night');roundTitle.textContent=stageLabel(round);roundDate.textContent=(dateLabel(round.scheduledOn));const _dl=deadlineHint(round.lineupDeadlineAt||round.lineup_deadline_at);if(_dl)roundDate.textContent=String(roundDate.textContent||'')+(' · '+_dl);const matchStatuses=(round.matches||[]).map((m)=>m.status);let derivedStatus=round.status;if(matchStatuses.length){if(matchStatuses.every((s)=>s==='finalized'||s==='corrected'))derivedStatus='finalized';else if(matchStatuses.some((s)=>s==='in_progress'||s==='finalized'||s==='corrected'))derivedStatus='in_progress'}roundStatus.textContent=statusLabel(derivedStatus);roundStatus.className='status-pill';roundStatus.dataset.tone=statusTone(derivedStatus);if(window.fdStableList){
          window.fdStableList(matchList,round.matches||[],{
            ...
            render:(match)=>{
              const card=document.createElement('article');card.className='match';card.dataset.status=String(match.status||'scheduled');const top=document.createElement('div');top.className='match-top';const table=document.createElement('span');table.textContent='Table '+(match.tableNumber||'—');const state=document.createElement('span');state.className='status-pill';state.dataset.tone=statusTone(match.status);state.textContent=statusLabel(match.status);top.append(table,state);const versus=document.createElement('div');versus.className='versus';const teamA=document.createElement('strong');teamA.textContent=match.teamAName;const vs=document.createElement('span');vs.textContent='vs';const teamB=document.createElement('strong');teamB.textContent=match.teamBName;versus.append(teamA,vs,teamB);const actions=document.createElement('div');actions.className='match-actions';const score=document.createElement('a');const finalized=match.status==='finalized'||match.status==='corrected';const live=match.status==='in_progress';score.className=finalized?'':'primary';score.href='/scorecard?match='+encodeURIComponent(match.teamMatchId||'');score.textContent=finalized?'View final':(live?'Score live':'Score');score.setAttribute('aria-label',score.textContent+' — '+match.teamAName+' vs '+match.teamBName);const messages=document.createElement('a');messages.href='/messages?matchup='+encodeURIComponent(match.teamMatchId||'');messages.textContent='Messages';const lineup=document.createElement('a');lineup.href='/lineup?match='+encodeURIComponent(match.teamMatchId||'')+'&round='+encodeURIComponent(match.roundId||round.roundId||'');lineup.textContent='Lineup';lineup.setAttribute('aria-label','Lineup — '+match.teamAName+' vs '+match.teamBName);actions.append(score,lineup,messages);
        ...
        card.append(top,versus,actions,makeup,protocol);if(window.fdSafeAutocomplete)window.fdSafeAutocomplete.scan(card);return card;
          ...
          matchList.replaceChildren();for(const match of round.matches){const card=document.createElement('article');card.className='match';card.dataset.status=String(match.status||'scheduled');const top=document.createElement('div');top.className='match-top';const table=document.createElement('span');table.textContent='Table '+(match.tableNumber||'—');const state=document.createElement('span');state.className='status-pill';state.dataset.tone=statusTone(match.status);state.textContent=statusLabel(match.status);top.append(table,state);const versus=document.createElement('div');versus.className='versus';const teamA=document.createElement('strong');teamA.textContent=match.teamAName;const vs=document.createElement('span');vs.textContent='vs';const teamB=document.createElement('strong');teamB.textContent=match.teamBName;versus.append(teamA,vs,teamB);const actions=document.createElement('div');actions.className='match-actions';const score=document.createElement('a');const finalized=match.status==='finalized'||match.status==='corrected';const live=match.status==='in_progress';score.className=finalized?'':'primary';score.href='/scorecard?match='+encodeURIComponent(match.teamMatchId||'');score.textContent=finalized?'View final':(live?'Score live':'Score');score.setAttribute('aria-label',score.textContent+' — '+match.teamAName+' vs '+match.teamBName);const messages=document.createElement('a');messages.href='/messages?matchup='+encodeURIComponent(match.teamMatchId||'');messages.textContent='Messages';actions.append(score,messages);
        const makeup=document.createElement('div');
        makeup.className='makeup';
        const makeupState=match.makeupStatus||match.makeup_status;
        const makeupOn=match.makeupOn||match.makeup_on;
