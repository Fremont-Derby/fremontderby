...
function renderManagement(data,scorable){renderLeagueNightHub(data,scorable);renderInvitations(data.invitations||[]);renderCaptainTeams(data.captain_teams||[]);renderApplications(data.applications||[]);renderReturningSlots(data.returning_slots||data.returningSlots||[]);renderMembershipPractice(data.membership_practice||data.captain_teams||[])}function renderMembershipRequests(data){renderJoinTeams(data.joinable_teams||[]);renderPlayerRequests(data.player_requests||[]);renderCaptainRequests(data.captain_requests||[])}
    ...
    async function loadPublicTeamDirectory(){
      ...
      try{
        ...
        setStatus((window.fdFriendlyError?window.fdFriendlyError(error):(error.message||'Sign in to manage teams.')),'muted');
      }
    }
    if(accessToken()||isOpenAuthLane())loadInitialTeams();else loadPublicTeamDirectory()

    if(hubReadyCheck){hubReadyCheck.addEventListener('click',async()=>{const teamId=hubReadyCheck.dataset.teamId;const roundId=hubReadyCheck.dataset.roundId;if(!teamId||!roundId){setStatus('Pick a published matchup first so the ready check has a league night.','error');return}hubReadyCheck.disabled=true;if(hubReadyCheckCta)hubReadyCheckCta.textContent='Sending…';try{const token=sessionStorage.getItem('fd.accessToken')||'';if(!token)throw new Error('Sign in to start a ready check.');let response=await fetch('/api/teams/'+encodeURIComponent(teamId)+'/ready-checks',{method:'POST',headers:{authorization:'Bearer '+token,'content-type':'application/json'},body:JSON.stringify({roundId})});
if(response.status===404||response.status===405){
  response=await fetch('/api/ready-checks',{method:'POST',headers:{authorization:'Bearer '+token,'content-type':'application/json'},body:JSON.stringify({teamId,roundId})});
}
if(response.status===404||response.status===405){
  response=await fetch('/api/teams/ready-checks',{method:'POST',headers:{authorization:'Bearer '+token,'content-type':'application/json'},body:JSON.stringify({teamId,roundId})});
}
const body=await response.json().catch(()=>({}));if(!response.ok)throw new Error(body.error||'Could not start ready check');setStatus('Ready check sent — teammates will see a prompt when they open the app.','ok');if(hubReadyCheckCta)hubReadyCheckCta.textContent='Ready check sent';}catch(error){setStatus((window.fdFriendlyError?window.fdFriendlyError(error):(error.message||'Could not start ready check')),'error');if(hubReadyCheckCta)hubReadyCheckCta.textContent='Send ready check →';hubReadyCheck.disabled=false}})}
