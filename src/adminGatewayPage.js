...
<body>
  ...
  <script>
    ...
    function show(target){
      loading.hidden=true;adminContent.hidden=true;playerContent.hidden=true;signedOut.hidden=true;
      target.hidden=false;
    }
    ...
    async function resolveAccess(){
      ...
      try{
        ...
        if(response.status===401){show(signedOut);status.textContent='Your sign-in may have expired.';return;}
        if(response.status===403){show(playerContent);return;}
        throw new Error('access check failed');
      }catch{
        show(playerContent);
        status.dataset.tone='error';
        status.textContent='We could not check admin access. No league data was changed.';
      }
    }
