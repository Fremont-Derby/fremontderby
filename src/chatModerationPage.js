...
<body>
  ...
  <script>
    ...
    listEl.addEventListener('click',event=>{const button=event.target.closest('[data-action]');if(!button)return;const card=button.closest('[data-report-id]');moderate(card,button.dataset.action).catch(error=>setStatus(error.message,'error'))});
    load();
