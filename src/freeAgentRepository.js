...
export function createFreeAgentRepository(env, { fetch: fetchImpl = globalThis.fetch } = {}) {
  ...
  return {
    ...
    async listSeasonFreeAgents({ seasonId }) {
      ...
      // Beta fallback: players with no active team membership this season (not yet registered free_agent).
      // Prefer season_players when present so explicit FA registration stays authoritative.
      ...
      return (Array.isArray(players) ? players : [])
        .filter((p) => p?.id && !rostered.has(p.id))
        .map((p) => ({
          playerId: p.id,
