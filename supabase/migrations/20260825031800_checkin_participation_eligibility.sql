-- #1899: a rostered player is an active season participant even if an older
-- season_players registration row is absent. Free-agent registration continues
-- to use season_players. Keep the service-role-only RPC boundary unchanged.
...
comment on function public.get_own_date_availability(uuid, uuid, date) is
  'Service-role-only read of the authenticated actor date availability. Active registration or active team roster membership counts as season participation; missing availability remains unmarked.';
comment on function public.set_own_date_availability(uuid, uuid, date, text) is
  'Service-role-only write of the authenticated actor date availability for a scheduled league date. Active registration or active team roster membership is required.';
