...
create index audit_events_entity_idx
  on private.audit_events (entity_type, entity_id, created_at);

create index audit_events_actor_idx
  on private.audit_events (actor_user_id, created_at);
...
begin
  ...
  insert into private.audit_events (
    ...
    jsonb_build_object(
      'status', 'active',
      'roundCount', inserted_round_count,
      'teamMatchCount', inserted_match_count
    )
  );
...
comment on table private.audit_events is
  'Private immutable audit log for privileged league operations.';

comment on function public.publish_season_schedule(uuid, uuid, text, jsonb) is
  'Service-role-only transaction boundary for publishing and auditing a validated Season 1 schedule by a league admin.';
