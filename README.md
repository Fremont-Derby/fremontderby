...
- **#553 public season selection / QA isolation** — PR #561 adds an explicit `league` versus `qa` season-purpose boundary and normal public registration/standings/prize reads admit only league-purpose seasons. PR #562 keeps Standings controls disabled while seasons are still loading. #553 remains open for one deterministic lifecycle-aware default-selection policy across Schedule, Standings, and Prizes among eligible league seasons; explicit user selection remains authoritative.
...
Normal public season read models exclude seasons classified for QA purpose after PR #561. Schedule, Standings, and Prizes keep their existing page ownership; #553 only standardizes which eligible league season they choose by default when the user did not choose one explicitly.
