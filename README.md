...
- **#406 team-slot governance** — team entry uses four plain-language states: **Forming, Qualified, Accepted, Waitlisted**. PR #411 enforces initial qualification for new/current-season teams as an assigned captain plus at least 3 active rostered players before they can take a slot. Returning reservation/release is #408, persistent waitlist ordering/promotion is #409, participant-facing state plus 4-player opening-night depth is #410, and #448 owns visible in-context explanation when a Season Teams slot action is unavailable. Do not create a parallel team-registration page.
...
- **#382 site-wide visual/accessibility system** — the approved product language is warm/light page content with white/light surfaces and deep Fremont-green actions; the dark felt/wood treatment is reserved for shared navigation identity. PR #432 semanticized the shared design-system layer, fixed shared focus/placeholder contrast, and added reduced-motion plus forced-colors guardrails. Page-specific visual/accessibility sign-off remains tracked by #427/#428/#429/#434/#435/#437 rather than by parallel theme systems.
...
- `/rules` — authoritative public rules; PR #441/#436 completed the current postseason-rule copy reconciliation, while #437 owns remaining visual/accessibility sign-off without changing rule semantics
...
- `/schedule` — league dates, matchup context, and canonical personal dated Available / Unsure / Unavailable check-in after PR #376/#370; captain lineup/sub discovery consumes the same dated source after PR #389. PR #415 only compacts the existing three-state phone control and does not change page ownership
- `/availability` — **transitional duplicate availability surface** pending final #370 parity/recovery proof and retirement
- `/lineup` — authoritative captain blind-lineup workflow. PR #431 lets Score hand an unrevealed selected captain matchup here through **Prepare lineup**; dated availability, available-sub discovery, three-slot ordering, validation, lock/reveal, and persistence remain owned here. After PR #444, the production route and Captain War Games consume the same shared blind-lineup component/controller; visual/accessibility sign-off is #435
...
- `/standings` — team / individual standings; #434 owns remaining light-theme/accessibility sign-off without changing standings calculations or page ownership
...
- `/demo` — public **Test Drive the App** using fictional, non-authoritative data; production-component parity and the complete workflow inventory are reconciled under #387/PR #446, while remaining public visual/accessibility sign-off is #437
- `/sandbox/captain` — fictional captain practice child flow; after PR #444 its lineup drill uses the exact production blind-lineup component/controller with a fictional adapter, while formation/availability/churn remain clearly labeled sandbox orientation
...
The shared visual contract is likewise centralized: normal application content is warm/light, while dark felt/wood is reserved for the shared navigation shell. PR #432 adds the semantic compatibility/accessibility layer; page-local legacy styles are allowed only as transitional implementation detail while #427/#428/#429/#434/#435/#437 complete browser sign-off.
