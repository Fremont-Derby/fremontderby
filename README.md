...
- **#387 Test Drive component parity** — Test Drive / War Games is a QA harness around production components, not a parallel demo implementation. PR #386 made `/scorecard/live` and `/sandbox/player` share the rack-ledger component/controller; PR #444 now makes production `/lineup` and the lineup portion of `/sandbox/captain` share the exact blind-lineup component/controller with only live versus fictional adapters swapped. Remaining #387 work is the inventory/classification of other Test Drive actions, not scoring or lineup parity.
...
- `/rules` — public rules; #436 owns current postseason-rule copy reconciliation and #437 owns remaining visual/accessibility sign-off without changing rule semantics
...
- `/demo` — public **Test Drive the App** using fictional, non-authoritative data; simulated production actions should share production components under #387; remaining public visual/accessibility sign-off is #437
