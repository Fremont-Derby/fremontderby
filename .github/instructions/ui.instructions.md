...
- **Mandatory clarity invariant:** whenever two or more similar entities could be confused, participant-facing status, confirmation, warning, error, destructive action, and completion text must name the concrete entity. Do not rely on generic labels such as `Your`, `Opponent`, `This team`, `That player`, or position/color alone when the actual identity is known.
- This concrete-identity rule applies across the whole product, including team-vs-team and player-vs-player workflows, seasons/weeks/rounds, payment state, scoring, lineup submission and locking, substitutions, confirmations, corrections, destructive actions, and admin operations. Prefer forms such as `JFL QA Bank Shots — Submitted` over `Your lineup — Submitted`, and preserve enough identity on narrow screens that the user never has to infer which entity a state belongs to.
