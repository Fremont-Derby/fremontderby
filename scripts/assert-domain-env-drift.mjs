...
export function collectDomainEnvDrift({ wranglerConfig } = {}) {
  ...
  for (const hostname of Object.keys(hostMap)) {
    if (!LANE_HEALTH_CHECKS.some((c) => c.host === hostname)) {
      errors.push(`HOST_ENVIRONMENT_EXPECTATIONS ${hostname} missing from LANE_HEALTH_CHECKS`);
    }
  }
  ...
  for (const hostname of Object.keys(hostMap)) {
    if (!LANE_CUSTOM_DOMAINS.some((r) => r.hostname === hostname)) {
      errors.push(`HOST_ENVIRONMENT_EXPECTATIONS ${hostname} missing from LANE_CUSTOM_DOMAINS`);
    }
  }
