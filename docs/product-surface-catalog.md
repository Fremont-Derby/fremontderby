...
| `/teams` | Player / captain | Find, join, create, and manage teams and roster relationships | Canonical team/roster surface; phone-width table reflow shipped in PR #290 / #131 |
...
| `/standings` | Public visitor / player | View team and individual standings and season results | Canonical standings surface. PR #302 now distinguishes loading, legitimate no-season, empty, and load-failure states with direct recovery. Historical season selection remains #180 |
...
| `/admin/operations` | League admin / director | See readiness, exceptions, prioritized actions, and operational health | Canonical admin operations/triage surface. Shipped in PRs #292-#294; role-aware discovery from Profile shipped in PR #297. #169 remains open for remaining metric/status rules |
...
| Player / captain | Find/create/join/manage team relationships | `/teams` | Implemented | #131, #181, #182, PR #290 | Teams remains the canonical roster/membership home; league-night links are secondary shortcuts |
...
| League admin / director | See readiness and operational exceptions without querying Supabase | `/admin/operations` | Partial but shipped and discoverable | #168, #169, #239, PRs #292-#294/#297 | Profile -> Operations is <=2 actions after role authorization |
...
- **Profile status reconciled:** PR #301 completed the user-facing Profile state/recovery story and narrow-phone team/season history reflow. #298 and #299 are closed; the catalog no longer carries #299 as an active gap.
- **Standings status reconciled:** PR #302 completed the current Standings loading/no-season/load-failure recovery slice without adding a new route or changing canonical ownership. #250 should now track only the remaining route-state work rather than treating Standings as untouched.
- **Admin ownership remains clean:** `/admin/operations`, `/season-setup`, and `/messages/moderation` retain distinct primary jobs. Role-aware Profile discovery keeps the admin toolset within the two-action rule without exposing it to ordinary players.
- **Known navigation-copy gap remains:** `src/appShell.js` still labels `/demo` as `Demo` while the canonical public product name is **Try a League Night**. #249 already owns this; do not create a duplicate issue.
- **No new routes or duplicate surfaces** were introduced by PRs #301/#302.
- **No overlapping open PRs** existed during this review cycle.
...
- #250 — route-state cleanup. Score, Messages, Profile, and Standings recovery slices are shipped; reconcile remaining Teams/shared-state work before closure.
- #169 — remaining `/admin/operations` metric/status acceptance work.
