...
| League admin / director | See readiness and operational exceptions without querying Supabase | `/admin/operations` | Partial but shipped and discoverable | #168, #169, #239, PRs #292-#294/#297/#304 | Profile -> Operations is <=2 actions after role authorization; lineup-deadline action cards link directly to `/lineup` |
...
- **Operations readiness expanded without changing page ownership:** PR #304 added current-round lineup counts plus warning/critical lineup-deadline action cards to `/admin/operations`. Operations remains the canonical league-readiness/exception surface; direct action links to `/lineup` support the existing two-action discoverability model rather than creating a new admin workflow page.
- **Teams remains the one unfinished major route-state slice:** #305 now explicitly owns signed-out/loading/empty/expired-session/failure presentation on `/teams`; no duplicate team or registration surface should be created for that work.
...
- **No new routes or duplicate surfaces** were introduced by PR #304.
...
- #169 — remaining `/admin/operations` operational signals after the shipped availability and lineup-deadline slices.
