...
| `/availability` | Player / captain | Declare and review dated league-night availability/check-in | Canonical availability surface. PR #318 shipped truthful loading/signed-out/session/failure recovery. #138 now owns the dated Available/Unavailable/Unsure check-in semantics and fast captain substitute discovery |
...
| `/scorecard/live` | Player | Operate active team-owned rack scoring | Canonical live-scoring surface. PR #331 shipped shared 8-first/9-first opening discipline and PR #333 rebuilt the first view around running team score, individual race, and aligned dual-submission rack ledger. Per-rack edit/mismatch completion is active in PR #334 under #324/#325; final two-phone/War Games proof remains #326 |
...
| Player / team member | Choose opening discipline, compare rack submissions, score, reconcile, confirm, and finalize | `/scorecard/live` | Rack-ledger first view shipped; edit/mismatch QA active | #14, #73, #321-#326; PRs #331/#333; PR #334 open | Reduced shared shell is an intentional task-focus exception. Team-owned scoring remains canonical here; running team score and individual score are derived, not manually entered |
...
- **Availability recovery is now shipped:** PR #318 merged and closed #317. `/availability` remains the canonical check-in surface; the newer product decision in #138 is not yet a documentation-only tweak—it changes availability from round-centric assumptions to a calendar-date Available/Unavailable/Unsure model with Unsure by default, so implementation belongs in the owning feature/data lane.
...
- **Live scoring has a materially new first-view product shape:** PR #331 persisted shared 8-first/9-first discipline and PR #333 made the rack ledger, running team score, and aligned team-owned W/L submissions the canonical live scoring presentation. This remains one `/scorecard/live` surface, not a new route. PR #334 is the active per-rack edit/mismatch implementation and must not be documented as shipped until merged; #326 owns final War Games/two-phone proof.
- **Rack-ledger backlog needs state reconciliation:** #322 is complete/closed, #323 implementation is merged but its War Games parity criterion is delegated to #326, and parent #321 still shows all child checkboxes unchecked. The parent should distinguish shipped slices from active QA rather than making current `main` look untouched.
...
- #321 — rack-ledger live-scoring epic. #322 is complete; #323 implementation is on `main`; PR #334 actively overlaps #324/#325; #326 owns final War Games/two-phone proof.
