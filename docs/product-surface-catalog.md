...
| `/availability` | Player / captain | Declare and review league-night availability | Canonical availability surface; substitute-discovery improvements remain under #138. PR #318 is the active non-happy-path UX implementation and is not yet current `main` |
...
| `/admin/operations` | League admin / director | See readiness, exceptions, prioritized actions, and operational health | Canonical admin operations/triage surface. Shipped in PRs #292-#294; role-aware discovery from Profile shipped in PR #297; PR #304 added lineup-deadline warnings/critical actions; PR #307 added aged dual-score mismatch warning/critical actions; PR #309 added 90/150-minute started-but-unfinalized match escalation; PR #314 added a critical selected-player payment-eligibility signal. #169 remains open for remaining operational signals |
| `/admin/players` | League admin / director | Find a player and manage player-level league administration without technical IDs | Canonical player-administration surface introduced by PR #320 under #316. Current `main` supports player search, team-membership visibility, and league-admin grant/revoke with last-admin protection. Competition eligibility is active work in PR #327; roster exceptions remain open under #316 |
...
| Player / captain | Mark league-night availability | `/availability` | Implemented core; recovery PR open | #13, #138, #317, PR #318 | Teams -> Mark availability remains within two deliberate actions; do not mark PR #318 recovery shipped until merged |
...
| League admin / director | See readiness and operational exceptions without querying Supabase | `/admin/operations` | Partial but shipped and discoverable | #168, #169, #239, PRs #292-#294/#297/#304/#307/#309/#314 | Profile -> Operations is <=2 actions after role authorization; action links delegate execution to the canonical owning workflow rather than duplicating it |
| League admin / director | Find a player and manage player-level roles, eligibility, and roster exceptions | `/admin/players` | Partial but shipped | #316, PR #320; PR #327 open | Player search and admin-role management are on current `main`; Profile must expose Players directly for <=2-action discovery. Eligibility and roster-exception slices remain owned by #316 rather than Operations or Teams |
...
- **Admin player management is now a distinct canonical surface:** PR #320 introduced `/admin/players` for human-readable player search, team-membership visibility, and league-admin grant/revoke. This belongs separately from `/admin/operations`: Operations answers “what needs attention?”, while Players answers “manage this person.” #316 is the durable parent story; PR #327 is the active eligibility slice and roster exceptions remain unresolved there.
- **The initial `/admin/players` navigation path violated the <=2-action rule:** current `main` exposes Players from `/admin/operations`, while Profile's authorized admin panel exposes Operations, Season setup, and Moderation. That makes the ordinary shared-nav path Profile -> Operations -> Players. This librarian cleanup adds Players directly to the role-aware Profile admin panel, preserving the existing admin authorization probe and making Profile -> Players <=2 actions.
- **Operations gained another current-main readiness signal:** PR #314 flags current-round selected players who are no longer paid/waived and links recovery to the existing owning workflow instead of creating another administration surface.
- **Availability recovery is active but not merged:** PR #318 targets truthful signed-out/loading/session/failure states on `/availability`. It overlaps #250's cross-route recovery theme but remains owned by #317/#138 until merge; current-main documentation must not claim it shipped.
- **Teams non-happy-path recovery is shipped:** PR #312 completed #305 without changing team, membership, captaincy, registration, authorization, database, payment, lineup, or scoring semantics. `/teams` remains the one canonical team/roster/membership surface.
- **The major-route recovery epic is implementation-complete for its original five routes:** Score (PR #277), Messages (PR #285), Profile (PR #301), Standings (PR #302), and Teams (PR #312) all have focused recovery. #250 remains open only for its final cross-route consistency/shared-pattern audit; Availability's separate #317 work should be reconciled there after merge without reopening the original five slices.
...
- No duplicate surface was created by `/admin/players`: player-specific administrative mutation belongs there; league readiness/triage remains `/admin/operations`; normal roster membership work remains `/teams`.
...
- #250 — route-state cleanup. Score, Messages, Profile, Standings, and Teams recovery slices are shipped; only the final cross-route consistency/shared-pattern audit remains. Reconcile #317/#318 after merge as adjacent guidance, not as a regression of those completed slices.
...
- #316 — canonical `/admin/players` story for player-level league administration. PR #320 shipped search/admin-role management; PR #327 is the open eligibility slice; roster exceptions remain unresolved.
