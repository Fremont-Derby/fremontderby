...
test('availability page uses signed-in human-readable league-night selection', () => {
  ...
  assert.match(html, /No published league nights are ready for check-in/);
});
...
test('availability choices stay compact and expose selected state accessibly', () => {
  ...
  assert.match(html, /await signedApi\('\/api\/seasons\/'\+encodeURIComponent\(seasonId\)\+'\/availability\/me'/);
