...
test('resolveBuildLane defaults to production and accepts known lanes', () => {
  ...
  assert.equal(resolveBuildLane({}, 'gamma'), 'gamma');
});

test('resolveBuildLane rejects unknown lanes', () => {
  assert.throws(() => resolveBuildLane({ FREMONT_BUILD_LANE: 'staging' }), /unknown FREMONT_BUILD_LANE/);
});
