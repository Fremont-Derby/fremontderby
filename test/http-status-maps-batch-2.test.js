...
test('team membership request status map', () => {
  ...
  assert.equal(teamMembershipStatusForError(new Error('Membership request not found')), 404);
  assert.equal(teamMembershipStatusForError(new Error('Invitation not found')), 404);
...
test('admin season teams status map', () => {
  ...
  assert.equal(adminSeasonTeamsStatusFor(new Error('No team slots are currently available')), 409);
