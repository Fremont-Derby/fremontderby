...
test('Gamma practice repair prefers league-night makeup copy', () => {
  ...
  assert.doesNotMatch(repaired, /Practice nights will show up here/);
  assert.doesNotMatch(repaired, /Nothing is scheduled on this page yet/);
});

test('Gamma practice route HTML uses league-night makeup copy', async () => {
  ...
  assert.doesNotMatch(html, /Practice nights will show up here/);
});
