...
test('player profile repository fetches the actor profile by user id', async () => {
  const { fetch, calls } = createFetch([{
    body: [{
      id: 'player-1', user_id: 'user-1', display_name: 'Kai',
      fargo_rating: 531, rating_status: 'established',
      teams: [{ teamName: 'Breakers' }], seasons: [{ seasonName: 'Fall 2026' }],
    }],
  }]);
...
test('player profile repository upserts through the profile RPC', async () => {
  ...
  const repository = createPlayerProfileRepository(env, { fetch });
  const profile = await repository.saveProfile({ actorUserId: 'user-1', displayName: 'Kai B' });
  assert.equal(profile.display_name, 'Kai B');
  assert.match(calls[0].url, /upsert_player_profile/);
  const body = JSON.parse(calls[0].init.body);
  assert.equal(body.actor_user_id, 'user-1');
});

test('player profile repository surfaces Supabase failures', async () => {
  const { fetch } = createFetch([{ status: 400, body: { message: 'display_name is required' } }]);
  const repository = createPlayerProfileRepository(env, { fetch });
